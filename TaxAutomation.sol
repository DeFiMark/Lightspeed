 ...
 */
contract TaxAutomation is AutomationCompatible, Ownable {

    IFeeReceiver public info = IFeeReceiver(0xbb7dED6f759979b31e4973B13a913866cB305e06);
    address public constant token = 0x88dcADf011eA8faa30291151b97b1d8507ba490b;

    uint256 public minTrigger = 25 ether;
    ...
    function setFeeReceiver(address infoAddress) external onlyOwner {
        info = IFeeReceiver(infoAddress);
    }

    function checkUpkeep(bytes calldata) external override cannotExecute returns (bool upkeepNeeded, bytes memory performData) {
        uint256 balance = IERC20(token).balanceOf(address(info));
        upkeepNeeded = balance >= minTrigger;
