...
contract Distributor is Ownable, ReentrancyGuard {
    ...
    // WETH contract
    IWETH public constant WETH = IWETH(0x039e2fB66102314Ce7b64Ce5Ce3E5183bc94aD38);
    ...
    // 0.1 minimum sonic distribution
    uint256 public minDistribution = 0.1 ether;

    uint256 public minHolding = 5_000 ether;

    bool public canPublicDistribute = true;
    ...
    function approveTokenForSwapCustomRouter(address token, uint8 buyTax, address swapper) external onlyOwner {
        require(token != address(0), 'Zero Address');
        require(!rewardTokens[token].isApproved, 'Token Already Approved');
    ...
    function removeTokenFromSwap(address token) external onlyOwner {
    ...
    function claim(uint256 minOut) external nonReentrant {
        distributeDividend(msg.sender, minOut);
    }

    function batchClaim(address[] users, uint256[] minOuts) external {
    ...
    function addShareholder(address shareholder) internal {
        if (shareholders[shareholderIndexes[shareholder]] == shareholder) {
